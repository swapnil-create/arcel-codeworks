...
| D05 session auth gate | `lib/session.js`, `api/auth/*`, `api/chat.js`, [AUTH.md](./AUTH.md) | Signed session cookie required before OpenRouter spend. Fail-closed without `AUTH_SECRET` + OAuth env. In-memory isolation fixtures exist. **SEC-01 still Fail.** |

## Blocked before public generation

| Requirement group | Why it cannot be merged safely now | Required decision or infrastructure |
|---|---|---|
| SEC-01 / ACC-01 | Session cookie gate exists; in-memory isolation fixtures cover guessed IDs / revoked membership / cross-user reads. There is still no production IdP or Postgres membership store. | Owner: configure AUTH_SECRET + OAuth on Vercel ([AUTH.md](./AUTH.md) checklist) before OpenRouter on production; add personal-workspace schema and live ACC-01 tests. |
