...
| D05 honesty / CI / schema scaffold | `app.js`, `lib/generation-errors.js`, `.github/workflows/ci.yml`, `docs/PREVIEW-CI.md`, `docs/data-model/` | In progress on D05: blocked generation uses Flow G-style banners (`AUTH_REQUIRED`, API-not-configured); PR static checks; schema stubs. Persistence, auth provider, and protected preview remain **not built**. |
