...
const GATE_CTAS = Object.freeze([
  Object.freeze({ action: "sign-in", label: "Sign in" }),
...
const CATALOG = Object.freeze({
  AUTH_REQUIRED: Object.freeze({
    ...
    detail: "Generation requires a signed-in session. Unauthenticated requests cannot spend the provider key. This is not an assistant reply.",
    state: "permission-denied",
    retryable: false,
    ctas: GATE_CTAS
  }),
  AUTH_NOT_CONFIGURED: Object.freeze({
    code: "AUTH_NOT_CONFIGURED",
    httpStatus: 503,
    taxonomy: "auth_required",
    title: "Sign-in not configured",
    detail: "AUTH_SECRET and an OAuth provider are not configured on this deployment. Opening settings does not enable generation.",
    state: "permission-denied",
    retryable: false,
    ctas: GATE_CTAS
  }),
  PERMISSION_DENIED: Object.freeze({
    ...
    title: "Access denied",
    detail: "This object is not available to the signed-in principal. Guessed IDs and revoked memberships fail closed. This is not an assistant reply.",
    state: "permission-denied",
    retryable: false,
    ctas: GATE_CTAS
  }),
...
function hasSessionPrincipal(session) {
  return Boolean(session && typeof session.sub === "string" && session.sub.trim());
}

function generationGate(env = process.env, session = null) {
  ...
  // Session is the only spend authorization. OPENROUTER_ALLOW_UNAUTHENTICATED_DEMO
  // must never bypass this check — even when set to "true".
  if (!hasSessionPrincipal(session)) {
    return {
      status: CATALOG.AUTH_REQUIRED.httpStatus,
      code: "AUTH_REQUIRED",
      error: "Generation requires a signed-in session. Unauthenticated requests cannot spend the provider key."
    };
  }
  return null;
}
...
const api = { CATALOG, GATE_CTAS, classifyGenerationFailure, generationGate, attachErrorMeta, hasSessionPrincipal };
