...
async function exchangeGoogle(provider, origin, code) {
  ...
  if (!googleIdentityAllowed(user)) throw new Error("google_domain_restricted");
  return {
    sub: `google:${user.sub}`,
    provider: "google",
    name: user.name || "",
...
function googleIdentityAllowed(user) {
  if (!user || user.email_verified !== true || typeof user.email !== "string") return false;
  return /@arcelintelligence\.com$/i.test(user.email.trim());
}

module.exports = { authorizeUrl, exchangeCode, googleIdentityAllowed };
