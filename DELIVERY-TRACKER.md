...
| D05 | Identity, storage permissions, canonical data model, ledger, preview deploy pipeline | Coder + Standards | in progress | Foundation slice started: honesty/auth-required UX, PR CI scaffold, Conversation/Message/Run/UsageEntry stubs. Evidence: `app.js` banners · `lib/generation-errors.js` · `.github/workflows/ci.yml` · [docs/PREVIEW-CI.md](./docs/PREVIEW-CI.md) · [docs/data-model/README.md](./docs/data-model/README.md). **Not done** — no IdP, no live DB, no ledger execution, protected preview still needs owner GitHub↔Vercel + branch protection. |
