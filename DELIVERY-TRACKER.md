...
Last updated: 2026-09-14 (D05 SEC-01 isolation tests; visual overhaul approved) · Zone: Asia/Dubai (GST)
...
| D04 | Provider spikes: streaming/tools, search citations, image edit, voice; log gaps + cost | Coder | in progress | Visual overhaul approved ([PR #13](https://github.com/swapnil-create/arcel-codeworks/pull/13)). Backend resumed. Private streaming pass recorded; tools/search/image/voice remain `unsupported_capability`. **Not done.** |
| D05 | Identity, storage permissions, canonical data model, ledger, preview deploy pipeline | Coder + Standards | in progress | Visual overhaul approved; D05 backend resumed. Session/OAuth gate is on main ([PR #11](https://github.com/swapnil-create/arcel-codeworks/pull/11)). This slice strengthens session-gate + in-memory isolation tests. SEC-01 remains Fail. **Owner action:** set `AUTH_*` on Vercel ([AUTH.md](./docs/AUTH.md) checklist) before OpenRouter on production. |
...
|---|---|---|
| Coder | Visual overhaul approved. D04/D05 backend resumed (not done). Isolation tests this slice; IdP/DB/ledger still open. | in progress |
| UI UX Expert | ARCEL visual overhaul **approved** and merged via [PR #13](https://github.com/swapnil-create/arcel-codeworks/pull/13). Post-deploy visual QA next. | in progress |
