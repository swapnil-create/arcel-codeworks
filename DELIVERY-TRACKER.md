...
| D05 | Identity, storage permissions, canonical data model, ledger, preview deploy pipeline | Coder + Standards | in progress | Session cookie + OAuth scaffold on `/api/chat` (this branch): fail-closed without `AUTH_SECRET`/IdP; unauthenticated requests cannot spend. Honesty UX + PR CI + schema stubs already on main ([PR #8](https://github.com/swapnil-create/arcel-codeworks/pull/8)). Evidence: [docs/AUTH.md](./docs/AUTH.md) · `lib/session.js` · `api/auth/*`. **Not done** — SEC-01 still Fail (no IdP in prod, no cross-user ACL tests); no live DB; no ledger; protected preview still needs owner GitHub↔Vercel + branch protection. |
...
|---|---|---|---|---|
| WP-01 | Architecture, schemas, env, auth, CI | R0 decisions | in progress | Architecture + §20 gate docs **merged on main**: [WP-01-ARCHITECTURE.md](./docs/WP-01-ARCHITECTURE.md) via [PR #1](https://github.com/swapnil-create/arcel-codeworks/pull/1) (`ac69866`); [WP-01-SECURITY-NFR-GATE.md](./docs/WP-01-SECURITY-NFR-GATE.md) via [PR #3](https://github.com/swapnil-create/arcel-codeworks/pull/3). D05 P4 session gate in progress on this branch ([docs/AUTH.md](./docs/AUTH.md)). Still **not done** — SEC-01 Fail until prod IdP + cross-user tests; migration tests against a real DB + protected preview Fail; REL-03, OPS-01 remain. |
...
|---|---|---|
| Coder | D04 plan/harness via [PR #6](https://github.com/swapnil-create/arcel-codeworks/pull/6); private live streaming pass recorded in [D04-LIVE-RESULTS.md](./docs/D04-LIVE-RESULTS.md) ([PR #9](https://github.com/swapnil-create/arcel-codeworks/pull/9)). D04 still in progress — tools/search/image/voice unsupported. D05 honesty/CI/schema merged ([PR #8](https://github.com/swapnil-create/arcel-codeworks/pull/8)). D05 P4 session auth gate in progress on this branch. Remaining D05: prod IdP config, live DB, ledger, protected preview. Architecture notes merged ([PR #1](https://github.com/swapnil-create/arcel-codeworks/pull/1)). | in progress |
...
| 2026-09-14 16:05 | D05 P4 session auth gate (this branch): HMAC cookie + GitHub/Google OAuth; `POST /api/chat` requires a session. SEC-01 still **Fail**. Demo flag is not a spend bypass. |
